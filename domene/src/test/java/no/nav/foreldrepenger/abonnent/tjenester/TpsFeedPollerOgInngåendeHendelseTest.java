...
public class TpsFeedPollerOgInngåendeHendelseTest {
    ...
    public void skal_kunne_konvertere_json_fra_person_feed_til_hendelse() throws IOException {
        ...
        //Sjekk at det har blitt opprettet hendelse for den ene fødselshendelsen som ligger i json fil.
        List<InngåendeHendelse> inngåendeHendelser = hendelseRepository.finnHendelserSomErKlareTilGrovsortering();

        assertThat(inngåendeHendelser).hasSize(6); //Det er 1 FOEDSELSMELDINGOPPRETTET, 4 DOEDSMELDINGOPPRETTET og 1 DOEDFOEDSELOPPRETTET i eksempel json.
        assertThat(inngåendeHendelser).flatExtracting(InngåendeHendelse::getSekvensnummer).contains(41L);//41 er hentet fra eksempel json.
        assertThat(inngåendeHendelser).flatExtracting(InngåendeHendelse::getHåndtertStatus).containsOnly(HåndtertStatusType.MOTTATT);
        List<HendelsePayload> hendelsePayloadListe = inngåendeHendelseTjeneste.getPayloadsForInngåendeHendelser(inngåendeHendelser);
        ...
        assertThat(dødHendelseAktørIder).containsAll(asList("1155334466777", "7890123456789", "1234565432123", "1357911131723"));
