...
public class PollerTest {
    ...
    public void setUp() {
        ...
        inputFeed = InputFeed.builder().kode(FeedKode.UDEFINERT).navn("Unit test poller").ventetidFeilet("PT1S").ventetidFerdiglest("PT1S").ventetidLesbar("PT1S").feiletAntall(0).build();
    ...
    public void skal_logge_exception_ved_feil_ved_polling() {
        ...
        pollerSomFårNPE.run();
        logSniffer.assertHasWarnMessage("FP-142862:Kunne ikke polle database, venter til neste runde(runde=1): class java.lang.NullPointerException: null");
    }
    ...
    public void skal_behandle_VLEXception() {
        doThrow(AbonnentHendelserFeil.FACTORY.kanIkkeKonvertereFeedContent("type", 1).toException()).when(pollMedLoginTjeneste).poll(feedPoller, inputFeed);
        poller.run();
